...
- Filters: Plan (All Models / New Models = added within the last month, same window as the green (New) badge / Zen Models = on Zen pay-as-you-go, incl. Go-plan models marked `alsoOnZen` / Go Plan Models / OpenRouter Models) + "Available only" checkbox (hides `na` models), Min AA index, Min SWE-bench Pro, Min Terminal-Bench, Min DeepSWE, Max output ($/1M) — same order as the columns; Reset clears all.
