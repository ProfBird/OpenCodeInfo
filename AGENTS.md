...
- Update data:               `python update_zen_prices.py --output docs/models.json`   (adds/removes models, refreshes prices + SWE-bench Pro + Terminal-Bench + DeepSWE)
...
- Filters: Plan (All Models / Zen Models / Go Plan Models), Min AA index, Min SWE-bench Pro, Min Terminal-Bench, Min DeepSWE, Max output ($/1M) — same order as the columns; Reset clears all.
